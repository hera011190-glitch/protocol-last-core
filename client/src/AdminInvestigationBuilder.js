...
export default function AdminInvestigationBuilder({ goBack, initialInvestigationId = "" }) {
  ...
  const loadTemplate = (template) => {
    ...
    setBuilder({ id: template.id, title: template.title || "", type: template.type || "group", backgroundImage, bgmUrl, bgmVolume, start: startNodeId, entryCorrosion, nodes: nodes.length ? nodes : [createNode("start")] });
    ...
    <div style={{ padding: 26, color: "#13324b", display: "grid", gap: 18 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "340px 1fr 420px", gap: 18, alignItems: "start" }}>
        <section style={panelStyle}>
          ...
          <div style={{ display: "grid", gap: 6 }}><div style={{ fontSize: 12, fontWeight: 800, color: "#476885" }}>최초 진입 침식 진행도</div><input type="number" value={builder.entryCorrosion || 0} onChange={(e) => setBuilder((prev) => ({ ...prev, entryCorrosion: Number(e.target.value || 0) }))} placeholder="0" style={inputStyle} /></div>
