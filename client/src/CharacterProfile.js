...
function parseProfileSections(profileText) {
  ...
  if (current.title || current.lines.length) sections.push(current);
  if (sections.length === 0) sections.push({ title: "", lines: [String(profileText || "프로필이 없습니다.")] });
  return sections;
}
...
export default function CharacterProfile({ character, goBack, theme, viewerCharacter, design, pageKey = "profileCharacter" }) {
  ...
  const profileSections = parseProfileSections(character?.profile || "프로필이 없습니다.");
  ...
  const canRequestRelation = !!viewerCharacter?.id && String(viewerCharacter?.id) !== String(character?.id);
  ...
  const [audioVolume, setAudioVolume] = useState(() => {
    try {
      const raw = localStorage.getItem("plc-audio-volume");
      const num = Number(raw);
      return Number.isFinite(num) ? Math.max(0, Math.min(1, num)) : 0.72;
    } catch {
      return 0.72;
    }
  });
  ...
  useEffect(() => {
    ...
    if (profileBgm) {
      ...
      const handleVolume = (event) => setAudioVolume(Math.max(0, Math.min(1, Number(event?.detail?.volume ?? 0.72) || 0.72)));
      window.addEventListener("plc-audio-muted-changed", handleMuted);
      window.addEventListener("plc-audio-volume-changed", handleVolume);
      return () => {
        clearTimeout(retryTimer);
        window.removeEventListener("plc-audio-muted-changed", handleMuted);
        window.removeEventListener("plc-audio-volume-changed", handleVolume);
  ...
  const submitRelationRequest = async () => {
    if (!canRequestRelation) return;
    const res = await fetch("http://localhost:3001/relationRequests", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        fromCharacterId: viewerCharacter.id,
        fromCharacter: viewerCharacter.name,
        toCharacterId: character.id,
        toCharacter: character.name,
        relationName,
  ...
  return (
      ...
      style={{
        ...
        backgroundImage:
          background.image && pageOverlay !== "none"
            ...
            : pageOverlay !== "none"
            ? pageOverlay
            : "linear-gradient(180deg, rgba(250,254,255,0.92), rgba(239,248,255,0.92))",
    ...
    >
      ...
      {(pageDesign.elements || [])
        ...
        .map((element) => renderElement(element, {}, theme))}
      <div style={{ position: "relative", zIndex: 20, minHeight: "100vh", padding: "22px 24px 36px" }}>
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", display: "grid", gap: 24 }}>
        <div style={{ display: "grid", gridTemplateColumns: "minmax(300px, 0.9fr) minmax(0, 1.1fr)", gap: 24, alignItems: "start" }}>
          <div style={{ display: "grid", gap: 16, alignContent: "start" }}>
            <div style={{ width: "100%", maxWidth: 340, margin: "0 auto", display: "grid", gap: 10 }}>
              ...
              <div style={{ width: "100%", maxWidth: 340, margin: "0 auto", display: "flex", justifyContent: "center", paddingTop: 6 }}>
              ...
              <div style={{ display: "grid", gap: 4, textAlign: "left", alignItems: "start" }}>
                ...
                {canRequestRelation ? <button type="button" className="ghost-button" onClick={() => setRelationOpen(true)} style={{ marginTop: 6 }}>관계 신청</button> : null}
              </div>
            </div>

            <div style={{ width: "100%", maxWidth: 340, margin: "0 auto", display: "grid", gap: 10, alignContent: "start" }}>
          ...
          <div style={{ display: "grid", gap: 16, alignContent: "start" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(0, 1fr)", gap: 16 }}>
              <div style={{ padding: "18px 20px", borderRadius: 24, background: "rgba(255,255,255,0.72)", display: "grid", gap: 10, alignContent: "start", height: 188 }}>
                <div style={{ fontSize: 12, color: "#6a87a3", fontWeight: 800 }}>보유 스킬</div>
                <div style={{ color: "#4f7390", lineHeight: 1.8, whiteSpace: "pre-wrap", overflowY: "auto", paddingRight: 4 }}>
              ...
              <div style={{ display: "grid", gap: 16, alignContent: "start" }}>
                <div style={{ padding: "18px 20px", borderRadius: 24, background: "rgba(255,255,255,0.72)", display: "grid", gap: 8 }}>
                  ...
                  <div style={{ fontSize: 28, fontWeight: 900, color: "#13324b" }}>{Number(character?.coins || 0).toLocaleString()}</div>
                </div>
                <div style={{ padding: "18px 20px", borderRadius: 24, background: "rgba(255,255,255,0.72)", display: "grid", gap: 10, height: 108 }}>
                  <div style={{ fontSize: 12, color: "#6a87a3", fontWeight: 800 }}>보유 아이템</div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", overflowY: "auto", alignContent: "start", paddingRight: 4 }}>
        ...
        <div style={{ display: "grid", gap: 20 }}>
          {profileSections.map((section, index) => (
            <div key={`${section.title}-${index}`} style={{ color: "#4f7390", lineHeight: 1.95, whiteSpace: "pre-wrap" }}>
              {section.title ? <div style={{ fontSize: 22, fontWeight: 900, color: "#16324a", textAlign: "center", marginBottom: 10 }}>{`<${section.title}>`}</div> : null}
              <div>{renderProfileRichParagraph(section.lines.join("\n").trim() || "내용이 없습니다.")}</div>
            </div>
          ))}
        </div>
      ...
      {relationOpen && typeof document !== "undefined" ? createPortal((
        <div style={{ position: "fixed", inset: 0, background: "rgba(2,6,23,0.56)", display: "grid", placeItems: "center", padding: 24, zIndex: 2100 }} onClick={() => setRelationOpen(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: "min(560px, 100%)", borderRadius: 28, background: "rgba(255,255,255,0.98)", padding: 22, boxShadow: "0 26px 60px rgba(15,23,42,0.22)", display: "grid", gap: 12 }}>
            ...
            <div style={{ color: "#5d7a95", lineHeight: 1.7 }}>{viewerCharacter?.name} → {character?.name}</div>
            <input value={relationName} onChange={(e) => setRelationName(e.target.value)} placeholder="관계 이름" />
            <textarea value={relationDescription} onChange={(e) => setRelationDescription(e.target.value)} placeholder="설명" style={{ minHeight: 120, resize: "vertical" }} />
