...
export function ProfileCard({ character = {}, onClick, theme, width = "100%", oneLine, image, rank, name, frame, isOnline = false }) {
  ...
  const imageSrc = image ?? character?.mainImage ?? character?.image ?? "";
  ...
  return (
    ...
    >
      ...
      <div style={shapeStyle({ position: "absolute", inset: 0 })}>
        {imageSrc ? (
          <img src={imageSrc} alt={`${displayName}-full`} loading="eager" decoding="sync" fetchPriority="high" draggable={false} style={{ ...getProfileCardImageStyle(cardFrame), zIndex: 1 }} />
        ) : null}
      </div>
      <div
        style={shapeStyle({
