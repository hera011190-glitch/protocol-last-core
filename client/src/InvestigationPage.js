...
useEffect(() => {
  ...
  const handleStateUpdated = (payload) => {
    ...
    if (!hasRoundPlayback && !battlePlaybackLocked) {
      if (Array.isArray(payload.sharedLogs)) {
        setLogs(payload.sharedLogs);
      } else if (payload.sharedLog) {
        setLogs((prev) => [
          ...prev,
                      ...
                      <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                        ...
                        <div>
                          ...
                          <div style={{ marginTop: "6px", color: "#9fb0c7", fontSize: "13px" }}>
                            HP {hp}/{maxHp} / ATK {state.atk || participant.stats?.atk || 0} / DEF {state.def || participant.stats?.def || 0} / AGI {state.agi || participant.stats?.agi || 0}
                          </div>
